...
class Llama:
    ...
    def build(
        ...
        # local_rank = int(os.environ.get("LOCAL_RANK", 6))
        local_rank = 3
        ...
        print(f"Loaded to GPU in {time.time() - start_time:.2f} seconds")
        start_time = time.time()
        model.copy_to_cpu()
        print(f"Copied to CPU in {time.time() - start_time:.2f} seconds")
    ...
    def check(
        ...
        stop_tokens = torch.tensor(list(self.tokenizer.stop_tokens), device="cpu")
